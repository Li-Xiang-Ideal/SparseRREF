...
namespace SparseRREF {
	...
	template <typename T, typename index_t> struct sparse_tensor_struct {
		...
		// extract a (rank-1) tensor by fixing the first index, suppose that the tensor is already sorted by default
		sparse_tensor_struct<T, index_t> extract(const index_t index, thread_pool* pool = nullptr, const bool sorted = true) const {
			if (index < 0) {
			...
			for (size_t i = 0; i < rank - 1; i++)
				res_dims[i] = dims[i + 1];
			sparse_tensor_struct<T, index_t> B(res_dims, row_nnz(index));
			B.rowptr[0] = 0;
			if (pool == nullptr) {
				...
				if (!sorted) {
					std::vector<size_t> perm(B.nnz());
					for (size_t i = 0; i < B.nnz(); i++)
						perm[i] = i;
					std::sort(std::execution::par, perm.begin(), perm.end(), [&](size_t a, size_t b) {
						auto ptra = colptr + (rowptr[index] + a) * (rank - 1) + 1;
						auto ptrb = colptr + (rowptr[index] + b) * (rank - 1) + 1;
						return lexico_compare(ptra, ptrb, rank - 2) < 0;
					});
				...
				if (sorted) {
					...
					pool->detach_blocks(rowptr[index], rowptr[index + 1], [&](size_t ss, size_t ee) {
						for (size_t j = ss; j < ee; j++) {
							auto oldptr = colptr + (rowptr[index] + perm[j - rowptr[index]]) * (rank - 1);
							auto newptr = B.colptr + (j - rowptr[index]) * (rank - 2);
							auto id = SparseRREF::thread_id();
							row_block_nnz[oldptr[0]][id]++;
							std::copy(oldptr + 1, oldptr + (rank - 1), newptr);
							B.valptr[j - rowptr[index]] = valptr[rowptr[index] + perm[j - rowptr[index]]];
						}
					});
					pool->wait();
				}
		...
		sparse_tensor_struct<T, index_t> extract(const size_t levelspec, const index_t index, thread_pool* pool = nullptr, const bool sorted = true) const {
			if (levelspec == 0) {
